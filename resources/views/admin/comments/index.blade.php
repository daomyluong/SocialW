                    ...
                    <tr>
                        <td class="px-4 text-start">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <div class="fw-semibold {{ (int) ($comment->user_status ?? 1) === 0 ? 'text-muted text-decoration-line-through' : 'text-dark' }}">{{ $comment->author_name }}</div>
            ...
            <div class="modal-content admin-modal-shell">
                            ...
                            <div class="modal-panel-soft p-4 h-100" style="max-height: 100%; min-height: 520px; overflow-y: auto;">
                                ...
                                <div class="report-item mb-3">
                                    <div class="d-flex align-items-center mb-2">
                                        ...
                                        <div>
                                            <div class="fw-semibold text-dark {{ (int) ($comment->user_status ?? 1) === 0 ? 'text-decoration-line-through' : '' }}">{{ $comment->author_name }}</div>
                                            <small class="text-muted">USER #{{ $comment->commenter_user_id }}</small>
                                        </div>
                                    </div>
                ...
                <div class="modal-footer admin-modal-footer d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <div class="d-flex gap-2 flex-wrap justify-content-end">
                        @if((int) ($comment->user_status ?? 1) === 1)
                            <form action="{{ route('admin.comments.quick_ban', $comment->commenter_user_id) }}" method="POST" class="d-inline" onsubmit="return confirm('Khóa tài khoản người này ngay lập tức?');">
                                @csrf
                                <button type="submit" class="btn btn-confirm-grad">Khóa nhanh tài khoản</button>
                            </form>
                        @endif
