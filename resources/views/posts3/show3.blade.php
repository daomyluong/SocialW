...
<div class="container" style="max-width: 760px;">
    ...
    <div class="d-flex gap-2 mb-4 overflow-auto">
        ...
        <div onclick="openStory({{ $index }})" style="cursor:pointer;">
            @if($story->type == 'image')
            <img src="{{ asset($story->media_url) }}" width="80" height="120" style="object-fit:cover; border-radius:10px;">
            @else
            <video width="80" height="120" style="object-fit:cover; border-radius:10px;">
...
">
    ...
    <button onclick="prevStory()" style="position:absolute; left:20px; top:50%; z-index:3;">←</button>
    <button onclick="nextStory()" style="position:absolute; right:20px; top:50%; z-index:3;">→</button>

    <div id="storyContent" style="z-index:2;"></div>
...
@section('scripts')
<script>
    let stories = {!! json_encode($stories ?? []) !!};
    let currentIndex = 0;
    ...
    function renderStory() {
        ...
        if (story.type === 'image') {
            html = `<img src="/${story.media_url}" style="max-height:80vh;">`;
        } else {
            html = `
